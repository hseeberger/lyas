...
/**
  * Provides the HTTP endpoint `/` from which a finite source of servers-sent
...
object LyasServer {
  ...
  private def route(charsPerSecond: Int, log: LoggingAdapter) = {
    ...
    def toServerSentEvent(c: Char, n: Int) =
      ServerSentEvent(data = Some(c.toString), id = Some(n.toString))
...
object LyasServerApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val charsPerSecond = config.getInt("lyas-server.chars-per-second")
    LyasServer(address, port, charsPerSecond)
